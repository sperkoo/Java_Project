...
public class AddTacheView extends JFrame {
    ...
    List<Liste> listes = gestionListe.getAllListes();
    List<Projet> projets = gestionProjet.getAllProjets();
    JLabel listesLabel = new JLabel("Pour ajouter cette tache a une liste selectionne sa nom:");
    JComboBox<Liste> listesComboBox = new JComboBox<>(listes.toArray(new Liste[0]));
    JLabel projetsLabel = new JLabel("Pour ajouter cette tache a un projet selectionne son nom:");
    JComboBox<Projet> projetsComboBox = new JComboBox<>(projets.toArray(new Projet[0]));

    public AddTacheView(boolean listeOrProjet) {
        ...
        if(listeOrProjet) {
            addToLayout(contentPane, projetsLabel, 0, 5, constraints);        
            addToLayout(contentPane, projetsComboBox, 0, 6, constraints);
        } else {
            addToLayout(contentPane, listesLabel, 0, 5, constraints);
            addToLayout(contentPane, listesComboBox, 0, 6, constraints);
        }
    ...
    public Projet getProjetsComboBox() {
        return (Projet) projetsComboBox.getSelectedItem();
    }
    
    public Liste getListesComboBox() {
        return (Liste) listesComboBox.getSelectedItem();
    }
}
